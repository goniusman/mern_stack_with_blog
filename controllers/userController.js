...
module.exports = {
    ...
    register(req, res) {
        ...
        let validate = registerValidator({ name, email,role, password, confirmPassword })
    ...
    imageUpload(req, res){
        ...
        file.mv(`${__dirname}/../client/public/user/uploads/${file.name}`, err => {
          if (err) {
            console.error(err);
          ...
          User.findById(userId)
            .then(user => {  
                    ...
                    User.findOneAndUpdate({ _id: userId }, { $set: user }, {new: true})
                        .then(result => { 
                            let token = jwt.sign({
                                _id: user._id,
                                name: user.name,
